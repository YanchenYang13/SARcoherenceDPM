...
可选：对最终生成的 `*final.tif`（以及额外指定的 tif）执行阈值掩膜：
...
- `--mask-method manual`：手动阈值，配合 `--mask-threshold-manual <value>`
- `--mask-method quantile`：分位数阈值，默认 `--mask-quantile 0.70`
- `--mask-method std`：`均值 + n*标准差`，默认 `--mask-std-n 2.0`

可通过 `--mask-input-tif a.tif b.tif` 追加“其他 tif 文件”做同样掩膜。
